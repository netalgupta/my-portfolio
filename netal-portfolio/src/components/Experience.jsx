...
const items = [
  { date: 'Summer 2026 · Current', role: 'Software Development Intern', company: 'SWDC — Software Development Working Committee', desc: 'Building cross-platform mobile apps with Flutter. Shipping real features for real users.', tag: '🔥 Current', tagBg: '#CAED4C', tagColor: '#0D0D0D', avatar: '👨‍💻' },
...
export default function Experience() {
  ...
  return (
    <section id="experience" style={{ padding: '7rem 4rem', background: 'var(--cream)' }}>
      <style>{`
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'start' }}>
        <div style={{ position: 'relative', paddingLeft: '2.5rem' }}>
          ...
          {items.map((item, i) => (
            <div key={i} className="tl-item reveal" style={{ paddingBottom: '2.8rem', position: 'relative' }}>
              ...
              <div className="tl-card">
                <div style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#E8341A', marginBottom: '0.3rem' }}>{item.date}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.2rem' }}>
                  <span style={{ fontSize: '1.2rem' }}>{item.avatar}</span>
                  <span style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.2rem', fontWeight: 700 }}>{item.role}</span>
                ...
                <div style={{ fontSize: '0.85rem', color: '#888', marginBottom: '0.5rem', fontWeight: 600 }}>{item.company}</div>
                <div style={{ fontSize: '0.88rem', lineHeight: 1.75, color: '#444', maxWidth: 440 }}>{item.desc}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="reveal" style={{ position: 'sticky', top: '8rem' }}>
          <div style={{ background: '#0D0D0D', borderRadius: 32, padding: '3rem', color: '#fff' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.6)', lineHeight: 1.8, fontSize: '0.95rem', marginBottom: '2rem' }}>
              Interning, shipping projects, competing in hackathons, and managing social media — all while maintaining a 9.8 SGPA.
            </p>
